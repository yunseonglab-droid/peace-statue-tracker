...
export async function resolvePoint(p:Place,options:{streetView?:boolean}={}):Promise<Point>{
 ...
 const types=options.streetView?['PointAddress','StreetAddress','POI']:['PointAddress','StreetAddress','StreetName','POI'];
 const result=candidates.find(c=>c.score>=95&&types.includes(c.attributes.Addr_type)&&Number.isFinite(c.location.x)&&Number.isFinite(c.location.y)&&c.location.x>=124&&c.location.x<=132&&c.location.y>=33&&c.location.y<=39.5);
 if(!result)throw new Error(`${p.name}: 가이드 위치를 자동으로 확인하지 못했습니다. 잘못된 장소로 연결하지 않도록 ${options.streetView?'거리뷰':'길찾기'} 연결을 중단했습니다.`);
 return {longitude:result.location.x,latitude:result.location.y};
}
// Selected in Naver's place detail and verified in its full panorama viewer.
export const naverStreetViews:Record<number,string>={
 1:'https://map.naver.com/p/entry/place/32594665?isMini=false&p=T1JeXT5IbQ1ekpOrjfoFPA,89,-3.91,80,Float',
};
export function buildNaverStreetViewUrl(point:Point){
 const {longitude,latitude}=point;
 if(!Number.isFinite(longitude)||!Number.isFinite(latitude)||longitude<124||longitude>132||latitude<33||latitude>39.5)throw new Error('거리뷰 위치가 올바르지 않습니다.');
 return `https://map.naver.com/p?isMini=false&p=${longitude},${latitude},Float`;
}
